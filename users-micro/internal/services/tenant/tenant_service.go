...
// TenantService maneja la lógica de tenants
...
func NewTenantService(db *gorm.DB, pub messaging.Publisher) *TenantService {
...
func (ts *TenantService) CreateTenant(ctx context.Context, tenantID string, tenantName string) error {
	logger.Info(fmt.Sprintf("Creando tenant: %s (%s)", tenantID, tenantName))
	...
	// Nota: No publicamos un evento aquí porque el evento de tenant creado
	// ya fue publicado por tenant-micro en el exchange tenants_events.
	// users-micro solo recibe y procesa ese evento a través de su consumer.
...
func (ts *TenantService) DeleteTenant(ctx context.Context, tenantID string) error {
	logger.Info(fmt.Sprintf("Eliminando tenant: %s", tenantID))
