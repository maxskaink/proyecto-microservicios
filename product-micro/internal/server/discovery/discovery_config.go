...
func (s *ServiceRegistration) Register(id string, name string, port int, tags []string) error {
	...
	logger.Info(fmt.Sprintf("Registrando servicio %s con IP %s y puerto %d", name, ip.String(), port))
...
func (s *ServiceRegistration) Deregister() error {
	...
	logger.Info(fmt.Sprintf("Desregistrando servicio %s", s.registration.ID))
